// dsh-beautify 动态版 · Host 半
// 用法：在 DSH 会话中 cordis_define，把本文件内容粘贴到 code.host；
// 再把 dynamic/client.js 粘贴到 code.client，然后 cordis_run 运行。
// 依赖：subprocess（Bing 壁纸抓取）、sandboxPolicy（工作区路径）、harness（RPC）
return {
  inject: ['subprocess', 'sandboxPolicy'],
  apply(ctx) {
...
fetch(url, { headers: { 'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' } })
  ...
  .catch(e => { console.log('[]'); });`
    ...
    // Client → Host RPC：关键词 → { label, tokens, candidates }

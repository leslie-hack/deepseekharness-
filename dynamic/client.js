// dsh-beautify 动态版 · Client 半
// 用法：在 DSH 会话中 cordis_define，把本文件内容粘贴到 code.client；
// 再配合 dynamic/host.js 一起运行（code.host）。cordis_run 后：
//   设置页：设置 → 界面美化·动态版；AI 头像与标题栏 logo 随图标上传自动出现。
// 与部署版（@local/dsh-beautify）可共存：本版使用独立 id 前缀 beautify-dyn。
return {
  inject: ['slots', 'theme'],
  apply(ctx) {
    ...
    const timer = timerService === undefined ? null : timerService
    ...
    function schedulePersist() {
      ...
      if (timer) {
        persistTimer = timer.timeout(function () { persistState(); persistTimer = null }, 400)
      }
    }
    ...
    function applyTokens(tokens) {
      ...
      currentDisposer = themeService.overrideTokens(SOURCE, effective)
      emitChange()
    }
    ...
    function cssVarRgba(name) {
      try {
        ...
        if (!v) return null
        // token 值可能是嵌套 var()（如 var(--dsw-static-...)），必须挂在 body 下让浏览器解析
    ...
    function probeImage(url, cb) {
      ...
      try {
        ...
        if (timer !== null) {
          timer.timeout(function () { finish(false) }, PROBE_TIMEOUT_MS)
        }
        img.src = url
    ...
    slots.inject('conversation.chat.assistant-actions', () => slots.register(
      { name: 'conversation.chat.assistant-actions', id: 'beautify-dyn-avatar', order: 1, label: 'AI 头像' },
      function () {
    ...
    slots.inject('conversation.session.header.utilities', () => slots.register(
      { name: 'conversation.session.header.utilities', id: 'beautify-dyn-logo', order: 100 },
      function () {
        const state = React.useState(getState)
        React.useEffect(function () {
          return subscribe(function () { state[1](getState()) })
    ...
    slots.inject('settings.section', () => slots.register(
      { name: 'settings.section', id: 'beautify-dyn', order: 35, label: '界面美化·动态版' },
      function () {
        const state = React.useState(getState)
        React.useEffect(function () {
          return subscribe(function () { state[1](getState()) })
        ...
        const onGenerate = async function () {
          ...
          if (res && res.ok) {
            generated = res
            generated.allCandidates = (res.candidates || []).filter(function (u) { return !!u })
            generated.wallpaperUrl = null
            const page0 = generated.allCandidates.slice(0, PAGE_SIZE)
            if (page0.length) {
              probing[1](true)
              probeAllLoadable(page0, function (okList) {
        ...
        const onNextPage = async function () {
          ...
          const all = generated.allCandidates || []
          const next = Math.min(Math.ceil(all.length / PAGE_SIZE) - 1, (pageState[0] || 0) + 1)
          pageState[1](next)
          const pageSlice = all.slice(next * PAGE_SIZE, (next + 1) * PAGE_SIZE)
          generated.wallpaperUrl = null
          probing[1](true)
          probeAllLoadable(pageSlice, function (okList) {
            generated.loadable = okList
            generated.wallpaperUrl = okList.length ? okList[0] : null
            probing[1](false)
            emitChange()
        ...
        const onPrevPage = async function () {
          ...
          pageState[1](prev)
          const pageSlice = all.slice(prev * PAGE_SIZE, (prev + 1) * PAGE_SIZE)
          generated.wallpaperUrl = null
          probing[1](true)
          probeAllLoadable(pageSlice, function (okList) {
            generated.loadable = okList
        ...
        const genSection = React.createElement('div', { className: 'bfy-gen' },
          ...
          s.generated ? React.createElement('div', { className: 'bfy-preview' },
            ...
            React.createElement('div', { className: 'bfy-preview-actions' },
              ...
              (function () {
                const total = s.generated && s.generated.allCandidates ? Math.ceil(s.generated.allCandidates.length / PAGE_SIZE) : 1
                const cur = Math.min((pageState[0] || 0) + 1, total)
                const noMore = cur >= total
                return React.createElement('button', { className: 'bfy-btn bfy-btn-ghost', disabled: busy[0] || noMore, onClick: onNextPage }, busy[0] ? '加载中…' : ('下一页 · ' + cur + '/' + total + ' 页'))
              })(),
        ...
        return React.createElement('div', { className: 'bfy-page' },
          React.createElement('h3', { className: 'bfy-title' }, '界面美化·动态版'),
          React.createElement('p', { className: 'bfy-desc' }, '与部署版功能一致；通过 cordis_run 直接运行，无需修改 cordis.patch.yml，进程重启后需重新运行。'),
          hueSection,
          genSection,
          studio,
          uploadSection,

...
export function apply(ctx) {
  ...
  webServer.register({
    ...
    async handler(req, res) {
      ...
      const candidates = []
      try {
        const bing = await bingWallpapers(sub, searchKw)
        for (const u of bing) {
          if (candidates.indexOf(u) < 0) candidates.push(u)
        }
      } catch (e) { /* 搜索失败 */ }
      // LoremFlickr 多变体（不同尺寸/seed 组合 → 不同图）
      ...
      res.end(JSON.stringify({ ok: true, label: t.label, tokens, candidates: candidates.slice(0, 50), hasMore: candidates.length > 10 }))
    },
  })

...
async function bingWallpapers(sub, keyword, page) {
  try {
...
export function apply(ctx) {
  ...
  webServer.register({
    ...
    async handler(req, res) {
      ...
      let candidates = []
      try {
        candidates = await bingWallpapers(sub, searchKw, page)
      } catch (e) { /* 搜索失败 */ }
      // 仅首页附带占位回退图；翻页时不注入，避免每页出现相同占位
      if (page === 0) {
        const fallback = [
      ...
      res.end(JSON.stringify({ ok: true, label: t.label, tokens, page, hasMore: candidates.length >= 10, candidates: candidates.slice(0, 20) }))
    },
  })
